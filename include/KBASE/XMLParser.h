...
class KBASE_API XMLNodeContents
{
public:
...
KBASE_API char* StringDup(const char* source, int cbData=0);
...
// be enlarged.
KBASE_API char* ToXMLString(const char* source);
KBASE_API char* ToXMLStringFast(char* *destBuffer,int *destSz, const char* source);

//---------------------------------------------------------------------------------------
// you should not use this one (there is a possibility of "destination-buffer-overflow"):
KBASE_API char* ToXMLString(char* dest,const char* source);
...
// the XML.
class KBASE_API KXMLParserBase64Tool
{
public:
