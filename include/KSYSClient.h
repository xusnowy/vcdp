...
#ifdef KSYSCLIENT_EXPORT
#define KSYSCLIENT_API _declspec(dllexport)
#elif KSYSCLIENT_DLL
#define KSYSCLIENT_API _declspec(dllimport)
#else
#define KSYSCLIENT_API
#endif

class KSYSCLIENT_API KSYSSession
{
public:
	KSYSSession(void){};
...
// 网络连接事件类
//=======================================================================================
class KSYSCLIENT_API KSYSSessionEvent
{
public:
...
KSYSCLIENT_API bool SYSSessionConnect(KSYSSessionEvent* pSYSSessionEvent,unsigned short usServerType,const std::string& strUserID);

/* 断开服务器连接 */
KSYSCLIENT_API void SYSSessionDisconnect(KSYSSessionEvent* pSYSSessionEvent);

/* 获得SYSSession实例 */
KSYSCLIENT_API KSYSSession& GetSYSSession(void);
...
class KSYSCLIENT_API KIMSSession
{
public:
	KIMSSession(void){};
...
public:
	 ...
	 * strStatus:   用户登录初始状态 
	 * strTermType: 终端类型 */
	virtual void Login(const std::string& strUserID,const std::string& strPassword,const std::string& strStatus,const std::string& strTermType="")=0;	
	...
	/* 添加联系人 */
	virtual void ApplyFriend(const std::string& strFriendID)=0;
...
// IMS模块事件类
//=======================================================================================
class KSYSCLIENT_API KIMSSessionEvent
{
public:
...
// IMS实例
//=======================================================================================
KSYSCLIENT_API bool IMSSessionAddEvent(KIMSSessionEvent* pIMSSessionEvent);
KSYSCLIENT_API void IMSSessionRemoveEvent(KIMSSessionEvent* pIMSSessionEvent);
KSYSCLIENT_API void IMSSessionRelease(void);
KSYSCLIENT_API KIMSSession& GetIMSSession(void);

#include "KMMSDefine.h"

class KSYSCLIENT_API KMMSSessionEvent;
//=======================================================================================
// MMS模块类
//=======================================================================================
class KSYSCLIENT_API KMMSSession
{
public:
	KMMSSession(void){};
	 ...
	 * szMetID: 临时会议名称
	 * 
	 * 返回值：本次请求的GUID值；用于上层能够确认是本次请求会话。【处理多个并发RequestTempMetRes发生的情况】
	 */
	virtual void RequestTempMetRes(const std::string& szMetName, std::string& szReqID)=0;
#endif // 2015
...
#if 1 // 2015.11.6
	/* 向指定临时会议中添加临时会议成员 */
	virtual void InsertTempMember(unsigned long ulRoomID, unsigned long ulGroupID, const std::string& strMemberID, const std::string& strMemberName, bool bPresider = false) = 0;
#endif // end
...
public:
	...
	virtual void OnTempMetRsp(const std::string& strDomain, unsigned long ulRoomID, unsigned long ulGroupID, const std::string& strGroupName, bool bDefault) = 0;
...
// MMS实例
//=======================================================================================
KSYSCLIENT_API bool MMSSessionAddEvent(KMMSSessionEvent* pMMSSessionEvent);
KSYSCLIENT_API void MMSSessionRemoveEvent(KMMSSessionEvent* pMMSSessionEvent);
KSYSCLIENT_API void MMSSessionRelease(void);
KSYSCLIENT_API KMMSSession& GetMMSSession(void);


#include "KMONDefine.h"

class KSYSCLIENT_API KMONSessionEvent;
//=======================================================================================
// MMS模块类
//=======================================================================================
class KSYSCLIENT_API KMONSession
{
public:
	KMONSession(void){};
...
// MON实例
//=======================================================================================
KSYSCLIENT_API bool MONSessionAddEvent(KMONSessionEvent* pMONSessionEvent);
KSYSCLIENT_API void MONSessionRemoveEvent(KMONSessionEvent* pMONSessionEvent);
KSYSCLIENT_API void MONSessionRelease(void);
KSYSCLIENT_API KMONSession& GetMONSession(void);
...
#if 0 // 2015
#include "KXSIPDefine.h"

class KSYSCLIENT_API KXSIPSessionEvent;
//=======================================================================================
// XSIP模块类
//=======================================================================================
class KSYSCLIENT_API KXSIPSession
{
public:
	KXSIPSession(void){};
	virtual ~KXSIPSession(void){};
	...
	/* 设备上线 */
	virtual void PBXOnline(const std::string &strPbxID,const std::string &strPwd,const std::string &strMask,const std::string &strName)=0;

	/* 设备下线 */
	virtual void PBXOffline(const std::string &strPbxID)=0;
	...
	/* 修改设备名称 */
	virtual void UpdateSipUserName(const std::string &strSipUserID,const std::string &strName)=0;

	/* 发送DVR通道上线信息 */
	virtual void SipUserOnline( const SIP_USER_ITEM &sipUserItem )=0;

	/* 发送DVR通道注销请求 chanelNum = -1 所有通道注销 */
	virtual void SipUserOffline( const std::string &strPbxID, const std::string& strSipUserNum )=0; 
	...
	/* 呼叫处理 */
	virtual void MakeInvite( const std::string &strCALLID,const DEV_CHANNEL_ITEM &channelItem,const std::string &strSDPData )=0;
	virtual void DoReject( const std::string &strCALLID,const SIP_USER_ITEM &sipUserItem )=0;
	virtual void DoAccept( const std::string &strCALLID,const DEV_CHANNEL_ITEM &channelItem,const std::string &strSDPData )=0;
	virtual void DoBye( const std::string &strCALLID )=0;
...
// XSIP模块事件类
//=======================================================================================
class KXSIPSessionEvent
{
public:
	KXSIPSessionEvent(void){};
	virtual ~KXSIPSessionEvent(void){};
...
public:
	...
	/* 设备上线 */
	virtual void OnPBXOnline(KCmdPacket& rCmdPacket)=0;

	/* 设备下线 */
	virtual void OnPBXOffline(KCmdPacket& rCmdPacket)=0;

	/* 收到通道上线 (DEVID,CHNUM,ERRCODE) */
	virtual void OnSipUserOnline(KCmdPacket& rCmdPacket)=0;

	/* 收到通道下线 */
	virtual void OnSipUserOffline(KCmdPacket& rCmdPacket)=0;
...
// XSIP实例
//=======================================================================================
KSYSCLIENT_API bool XSIPSessionAddEvent(KXSIPSessionEvent* pXSIPSessionEvent);
KSYSCLIENT_API void XSIPSessionRemoveEvent(KXSIPSessionEvent* pXSIPSessionEvent);
KSYSCLIENT_API void XSIPSessionRelease(void);
KSYSCLIENT_API KXSIPSession& GetXSIPSession(void);
#endif // 2015 end

//=======================================================================================
// AVC模块类
// 2015.9.24 Mark 标记 wangzwa 注：有关呼叫处理（呼叫请求）  ---- KAVCSessionEvent是呼叫响应
//=======================================================================================
class KSYSCLIENT_API KAVCSession
{
public:
	KAVCSession(void){};
...
// AVC模块事件类
//=======================================================================================
class KSYSCLIENT_API KAVCSessionEvent
{
public:
...
// AVC实例
//=======================================================================================
KSYSCLIENT_API bool AVCSessionAddEvent(KAVCSessionEvent* pAVCSessionEvent);
KSYSCLIENT_API void AVCSessionRemoveEvent(KAVCSessionEvent* pAVCSessionEvent);
KSYSCLIENT_API void AVCSessionRelease(void);
KSYSCLIENT_API KAVCSession& GetAVCSession(void);

//=======================================================================================
// WHB模块类
//=======================================================================================
class KSYSCLIENT_API KWHBSession
{
public:
	KWHBSession(void){};
...
// WHB模块事件类
//=======================================================================================
class KSYSCLIENT_API KWHBSessionEvent
{
public:
...
// WHB实例
//=======================================================================================
KSYSCLIENT_API bool WHBSessionAddEvent(KWHBSessionEvent* pWHBSessionEvent);
KSYSCLIENT_API void WHBSessionRemoveEvent(KWHBSessionEvent* pWHBSessionEvent);
KSYSCLIENT_API void WHBSessionRelease(void);
KSYSCLIENT_API KWHBSession& GetWHBSession(void);

//=======================================================================================
// SVR模块类
// 屏幕共享会话控制类
//=======================================================================================
class KSYSCLIENT_API KSRVSession
{
public:
	KSRVSession(void){};
...
// SVR模块事件类
//=======================================================================================
class KSYSCLIENT_API KSRVSessionEvent
{
public:
...
// SRV实例
//=======================================================================================
KSYSCLIENT_API bool SRVSessionAddEvent(KSRVSessionEvent* pSRVSessionEvent);
KSYSCLIENT_API void SRVSessionRemoveEvent(KSRVSessionEvent* pSRVSessionEvent);
KSYSCLIENT_API void SRVSessionRelease(void);
KSYSCLIENT_API KSRVSession& GetSRVSession(void);

//=======================================================================================
// FIL模块类
//=======================================================================================
class KSYSCLIENT_API KFILSession
{
public:
	KFILSession(void){};
...
// FIL模块事件类
//=======================================================================================
class KSYSCLIENT_API KFILSessionEvent
{
public:
...
// FIL实例
//=======================================================================================
KSYSCLIENT_API bool FILSessionAddEvent(KFILSessionEvent* pFILSessionEvent);
KSYSCLIENT_API void FILSessionRemoveEvent(KFILSessionEvent* pFILSessionEvent);
KSYSCLIENT_API void FILSessionRelease(void);
KSYSCLIENT_API KFILSession& GetFILSession(void);

//=======================================================================================
// SMS模块类
//=======================================================================================
class KSYSCLIENT_API KSMSSession
{
public:
	KSMSSession(void){};
...
// SMS模块事件类
//=======================================================================================
class KSYSCLIENT_API KSMSSessionEvent
{
public:
...
// SMS实例
//=======================================================================================
KSYSCLIENT_API bool SMSSessionAddEvent(KSMSSessionEvent* pSMSSessionEvent);
KSYSCLIENT_API void SMSSessionRemoveEvent(KSMSSessionEvent* pSMSSessionEvent);
KSYSCLIENT_API void SMSSessionRelease(void);
KSYSCLIENT_API KSMSSession& GetSMSSession(void);
...
class KSYSCLIENT_API KNFSSession
{
public:
	KNFSSession(void);
...
//---------------------------------------------------------
class KSYSCLIENT_API KNFSUploadSessionEvent
{
public:
...
//---------------------------------------------------------
class KSYSCLIENT_API KNFSUploadSession
{
public:
...
//---------------------------------------------------------
class KSYSCLIENT_API KNFSDownloadSessionEvent
{
public:
...
//---------------------------------------------------------
class KSYSCLIENT_API KNFSDownloadSession
{
public:
...
// 创建上传SESSION
//---------------------------------------------------------
KSYSCLIENT_API KNFSUploadSession* CreateNFSUploadSession(KNFSUploadSessionEvent& rNFSUploadSessionEvent);

//---------------------------------------------------------
// 创建下载SESSION
//---------------------------------------------------------
KSYSCLIENT_API KNFSDownloadSession* CreateNFSDownloadSession(KNFSDownloadSessionEvent& rNFSDownloadSessionEvent);
